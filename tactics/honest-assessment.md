...
4. **Log predictions before results.** Write the screen rank and base-rate estimate into `career/score-ledger.json` at submission, then compare at results. A ledger entry without a matching transcript or retro is labelled `"verified": false` and is never cited as calibration.
